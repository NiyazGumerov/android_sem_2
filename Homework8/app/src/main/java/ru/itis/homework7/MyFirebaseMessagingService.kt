...
import ru.itis.homework7.MainActivity
...
const val FROM_FIREBASE = "FROM_FIREBASE"
...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        if (remoteMessage.data.isNotEmpty()) {
            ...
            Log.d(TAG, "Message data payload: ${remoteMessage.data["Category"]}")
        ...
        when (remoteMessage.data["Category"]) {
            ...
            "Second" -> {
                userSharedPreferences.edit() {
                    putString(FROM_FIREBASE, remoteMessage.data["Title"].toString())
                }
            }
            ...
            "Third" -> {
                if (isAppInForeground()) {
                    CoroutineScope(Dispatchers.Default).launch {
                        NavigationEventBus.navigateTo(Route.FOR_FIREBASE_ROUTE)
                    }
                }
            }

            else -> {
                // Обработка случая, если категория не совпала ни с одной из перечисленных
            }
        }
    ...
    private fun isAppInForeground(): Boolean {
        // Пример проверки (нужно реализовать точно под проект)
